...
// //Connecting To Database : Mongoose Database
// mongoose
...
//   if(!name?.trim() || !email?.trim() || !password?.trim()) return res.status(401).json({message:"Fields are empty!", success:false})
...
//      if(!email?.trim() || !password?.trim()) return res.status(401).json({message:"Fields are empty!", success:false})
//   const existingUser = await UrlRegister.findOne({ email });
//   if (!existingUser)
...
//     process.env.JWT_SECRET, // here i wrote the secret key which i wrote in the env file i made for the databse connection
//     { expiresIn: "1h" } // here i wrote when this token will expire
//   );
//   res.status(200).json({
//     message: "Logged In",
//     success: true,
